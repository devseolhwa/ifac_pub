...
function showCalendar(_year, _month, _day, _stat) {
	...
	if (document.getElementById("inseqDatepicker")) {
		document.getElementById("inseqDatepicker").parentNode.removeChild(document.getElementById("inseqDatepicker"));
...
function removeDatepicker() {
	if ($("#inseqDatepicker").length > 0) {
		$("#inseqDatepicker").remove();
	}
}
$(document).on("click", ".btnDatepickerOpen", function (e) {
	e.preventDefault();
	openCalendar($(this).prev());
});
function setDatepicker(obj) {
	$(obj).wrap("<span class='datepicker'></span>");
	$(obj).after("<button type='button' class='btnDatepickerOpen'>달력</button>");
}